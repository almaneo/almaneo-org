...
export const WalletModal: React.FC<WalletModalProps> = ({
  ...
  const { address, balance, userInfo, disconnect } = useWeb3();

  // 현재는 빈 데이터 (추후 각 서비스에서 로드)
  const tokens: TokenBalance[] = [];
  const nftSummary: NFTSummary | null = null;
  const gameSummary: GameSummary | null = null;
  const stakingSummary: StakingSummary | null = null;
  const governanceSummary: GovernanceSummary | null = null;
